...
PAPMID_export_filtered<-as.data.frame(PAPMID_export %>% 
                                        group_by(Sample, file) %>% filter(DataCount == max(DataCount)) %>% ungroup())

# for the sample where the ID was repeates, keep the newer one
#Find all duplicates
...
#Subset to relevant
PAPMID_export_filtered['drop'] <- ifelse(PAPMID_export_filtered$repeated == TRUE & !grepl('20210219|20210224|Odense', PAPMID_export_filtered$file), 1, 0)
PAPMID_export_filtered <- PAPMID_export_filtered[PAPMID_export_filtered$drop == 0,]
PAPMID_export_filtered$drop<-NULL
#PAPMID_export_filtered$file<-NULL
PAPMID_export_filtered$repeated <- NULL
...
if (grepl('Spectratest', args[1])){
  ...
  output['strainnumber']<-ifelse(grepl('BRU', output$sample), gsub('(.*BRU\\-(WP1\\-)*)(\\d{1,2})(.*)', '\\3', output$sample2), 
                                 gsub('(.*)(\\-|\\_|\\.)(\\d{1,2}[[:alnum:]]*)(\\_mabr.*)', '\\3', output$sample2))
